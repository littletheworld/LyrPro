...
const SETUP_AUTOSAVE_KEY = 'lyric-sync-pro-setup-draft'; // For saving form progress
...
const SetupMode: React.FC<SetupModeProps> = ({ onStartSync, onLoadProject }) => {
  ...
  const [savedSetupDraft, setSavedSetupDraft] = useState<any>(null); // State for the setup draft
  ...
  useEffect(() => {
    const saveDraft = () => {
        ...
        const dataToSave: Record<string, any> = {
            ...
            artist,
            credits,
            audioFileName: selectedFile?.name || null,
            audioDataUrl: '',
            albumArtFileName: albumArtFile?.name || null,
            albumArtDataUrl: '',
        };
        ...
        const finalizeSave = () => {
            if (pendingReaders === 0) {
                localStorage.setItem(SETUP_AUTOSAVE_KEY, JSON.stringify(dataToSave));
            }
        };
        ...
        if (selectedFile) {
            ...
            reader.onload = (e) => {
                dataToSave.audioDataUrl = e.target?.result as string;
                pendingReaders--;
                finalizeSave();
            };
        ...
        if (albumArtFile) {
            ...
            artReader.onload = (e) => {
                dataToSave.albumArtDataUrl = e.target?.result as string;
                pendingReaders--;
                finalizeSave();
            };
  ...
  const handleProjectFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
        try {
            ...
            localStorage.removeItem(SETUP_AUTOSAVE_KEY); // Clear draft before loading project
            setRestoredFromSessionData(null); // Clear any restored session data as well
            onLoadProject(
                audioFile, 
                projectData.lyrics, 
                projectData.syncData,
                projectData.songTitle || '',
                projectData.artist || '',
                projectData.albumArtUrl || null,
                projectData.credits || ''
            );
        } catch (error) {
  ...
  const handleSubmit = useCallback(async () => {
    if (selectedFile && lyricsText.trim()) {
      localStorage.removeItem(AUTOSAVE_KEY); // Clear old session on new start
      localStorage.removeItem(SETUP_AUTOSAVE_KEY); // Clear draft on new start
      
      let artUrlForApp: string | null = null;
      if (albumArtFile) {
        artUrlForApp = await new Promise((resolve) => {
          const reader = new FileReader();
      ...
      if (restoredFromSessionData && JSON.stringify(currentLyricLines) === JSON.stringify(restoredFromSessionData.lyrics)) {
          // Lyrics are unchanged, reuse sync data and skip structure mode
          onLoadProject(
              selectedFile,
              restoredFromSessionData.lyrics,
              restoredFromSessionData.syncData,
              title,
              artist,
              artUrlForApp,
              credits
          ...
          // Lyrics changed (or it's a new project), go through normal flow
          onStartSync(selectedFile, lyricsText, title, artist, artUrlForApp, credits);
      }
      setRestoredFromSessionData(null); // Clear restored data after use
  ...
  const handleRestoreSession = useCallback(async () => {
    ...
    setRestoredFromSessionData({
        lyrics: savedSession.lyrics,
        syncData: savedSession.syncData,
    });

    localStorage.removeItem(SETUP_AUTOSAVE_KEY); // Clear draft
    localStorage.removeItem(AUTOSAVE_KEY); // Clear session from storage
    setSavedSession(null); // Hide the banner
  }, [savedSession]);
  ...
  const handleRestoreSetupDraft = useCallback(async () => {
    ...
    if (audioDataUrl && audioFileName) {
        try {
            const audioFile = await dataUrlToFile(audioDataUrl, audioFileName);
            setSelectedFile(audioFile);
        } catch (error) {
            console.error("Failed to restore audio file from draft:", error);
            setSelectedFile(null);
        }
    } else {
        setSelectedFile(null);
    ...
    if (albumArtDataUrl && albumArtFileName) {
        try {
            const artFile = await dataUrlToFile(albumArtDataUrl, albumArtFileName);
            setAlbumArtFile(artFile);
        } catch (error) {
            console.error("Failed to restore album art from draft:", error);
            setAlbumArtFile(null);
        }
    } else {
        setAlbumArtFile(null);
  ...
  const uploadAreaClasses = `border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all duration-300 ${
      selectedFile ? 'border-blue-400 bg-blue-50' : 'border-gray-300 hover:bg-gray-100 hover:border-gray-400'
  }`;
        ...
        <Card className="p-5 mb-6 bg-blue-50 border-blue-300">
            <div className="text-center">
                ...
                <div className="flex justify-center gap-4 mt-4">
                    <Button onClick={handleRestoreSetupDraft} variant="primary" title="ดำเนินการต่อจากฉบับร่างที่บันทึกไว้">
                        ทำต่อ
                    </Button>
                    <Button onClick={handleDismissSetupDraft} variant="secondary" title="ลบฉบับร่างนี้ทิ้งอย่างถาวร">
                        ลบทิ้ง
                    </Button>
                </div>
            </div>
        ...
        <Card className="p-5 mb-6 bg-yellow-50 border-yellow-300">
            <div className="text-center">
                ...
                <p className="text-sm text-yellow-700 mt-1">
                    "{savedSession.artist || 'Unknown'} - {savedSession.songTitle || 'Untitled'}"
                </p>
                <div className="flex justify-center gap-4 mt-4">
                    <Button onClick={handleRestoreSession} variant="primary" title="กู้คืนและทำงานต่อจากเซสชันที่แล้ว">
                        ทำงานต่อ
                    </Button>
                    <Button onClick={handleDismissSession} variant="secondary" title="ลบเซสชันที่บันทึกไว้นี้ทิ้ง">
                        ลบทิ้ง
                    </Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div className="md:col-span-1 flex flex-col gap-4">
                  <input 
                    ...
                    className="w-full p-3 bg-gray-50 text-gray-800 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:outline-none transition-colors duration-300"
                  />
                  <input 
                    type="text"
                    value={artist}
              ...
              <div className="md:col-span-1">
                 ...
                 <div 
                    className={`border-2 border-dashed rounded-xl w-full h-full min-h-[120px] flex items-center justify-center cursor-pointer transition-all duration-300 ${albumArtUrl ? 'border-blue-400 p-1' : 'border-gray-300 hover:bg-gray-100'}`}
              ...
              <div className="md:col-span-2">
                ...
                <div className={uploadAreaClasses} onClick={handleUploadClick}>
                  <p className={`font-semibold ${selectedFile ? 'text-blue-800' : 'text-gray-600'}`}>
                      {selectedFile ? `✓ ${selectedFile.name}` : '📁 คลิกเพื่ออัพโหลดไฟล์เสียง'}
                  </p>
                </div>
              </div>
            </div>
...
(ไม่มีเลข) = นักร้อง 1"
              className="w-full h-48 p-4 bg-gray-50 text-gray-800 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:outline-none resize-y transition-colors duration-300 placeholder:text-gray-400"
            />
          </div>
        </div>
        <div className="pt-2">
            <div className="text-center">
                <Button onClick={handleSubmit} disabled={!selectedFile || !lyricsText.trim()} icon={<Icons name="play"/>} title="เริ่มต้นการจัดโครงสร้างและซิงค์เนื้อเพลง">
                    เริ่มซิงก์
                </Button>
            </div>
            ...
            <div className="text-center">
                <button 
                    ...
                    title="เปิดไฟล์โปรเจกต์ (.lsk) ที่บันทึกไว้"
                    className="w-full sm:w-auto flex items-center justify-center gap-3 px-6 py-3 rounded-xl font-semibold text-base transition-all duration-200 ease-in-out bg-white border border-gray-300 text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                >
                    <Icons name="upload" className="w-5 h-5" />
                    <span>โหลดโปรเจกต์ที่เคยทำไว้</span>
                </button>
            </div>
        </div>
      </Card>
