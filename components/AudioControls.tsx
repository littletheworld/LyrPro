...
interface AudioControlsProps {
  ...
  isVisible?: boolean;
  onSaveToCloud?: () => void;
  cloudSaveState?: 'idle' | 'saving' | 'success' | 'error';
}
...
const AudioControls: React.FC<AudioControlsProps> = ({
  ...
  if (showPreviewControls) {
    ...
    const getCloudSaveButtonContent = () => {
        switch (cloudSaveState) {
            case 'saving': return 'กำลังบันทึก...';
            case 'success': return 'บันทึกแล้ว!';
            case 'error': return 'ผิดพลาด';
            default: return 'บันทึกบนคลาวด์';
        }
    };
      ...
      <div className={`fixed bottom-4 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl mx-auto p-3 rounded-2xl bg-black/30 backdrop-blur-md border border-white/10 shadow-lg z-50 flex flex-col items-center gap-3 transition-all duration-500 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-full pointer-events-none'}`}>
        ...
        <div className="w-full flex items-center gap-4 text-white">
            <div className="flex items-center gap-1">
                <button onClick={() => handleSkip(-10)} className={iconButtonClasses} aria-label="Rewind 10 seconds" title="ย้อนกลับ 10 วินาที"><Icons name="replay" className="w-5 h-5 transform scale-x-[-1]" /></button>
                <button onClick={handlePlayPause} className={iconButtonClasses} aria-label={isPlaying ? 'Pause' : 'Play'} title={isPlaying ? 'หยุดเล่น' : 'เล่น'}><Icons name={isPlaying ? 'pause' : 'play'} className="w-7 h-7" /></button>
                <button onClick={() => handleSkip(10)} className={iconButtonClasses} aria-label="Forward 10 seconds" title="ข้ามไปข้างหน้า 10 วินาที"><Icons name="replay" className="w-5 h-5" /></button>
            </div>
            <div className="flex-grow flex items-center gap-3">
                <span className="text-sm font-mono text-gray-200 w-12 text-center">{formatTime(currentTime)}</span>
                <input type="range" min="0" max={duration || 0} step="0.01" value={currentTime} onInput={handleSliderChange} onMouseDown={handleSeekStart} onMouseUp={handleSeekEnd} onTouchStart={handleSeekStart} onTouchEnd={handleSeekEnd} style={sliderStyle} className={`w-full h-2 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full ${sliderThumb}`} aria-label="Audio progress" />
                <span className="text-sm font-mono text-gray-200 w-12 text-center">{formatTime(duration)}</span>
            </div>
        </div>
        <div className="w-full flex items-center justify-center gap-2 flex-wrap">
            <button onClick={onToggleVocalRemover} className={`p-3 rounded-full transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-black/50 focus:ring-blue-500 ${isVocalRemoverOn ? activeToggleButtonClasses : inactiveToggleButtonClasses}`} title={isVocalRemoverOn ? 'ปิดโหมดคาราโอเกะ (ฟังเสียงร้อง)' : 'เปิดโหมดคาราโอเกะ (ตัดเสียงร้อง)'} aria-label={isVocalRemoverOn ? 'ปิดโหมดคาราโอเกะ (ฟังเสียงร้อง)' : 'เปิดโหมดคาราโอเกะ (ตัดเสียงร้อง)'} ><Icons name="microphone" className="w-5 h-5" /></button>
  ...
  const sliderStyle = { background: `linear-gradient(to right, #3b82f6 ${progressPercentage}%, #4B5563 ${progressPercentage}%)` };
  ...
  return (
    <div className={`p-4 rounded-lg border-t border-gray-200 bg-gray-100`}>
        <div className="flex items-center gap-3">
            ...
            <input type="range" min="0" max={duration || 0} step="0.01" value={currentTime} onInput={handleSliderChange} onMouseDown={handleSeekStart} onMouseUp={handleSeekEnd} onTouchStart={handleSeekStart} onTouchEnd={handleSeekEnd} style={sliderStyle} className={`w-full h-2.5 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full ${sliderThumb}`} aria-label="Audio progress" />
