...
export async function saveProjectFile(
  ...
  if (artist && songTitle) fileName = `${artist} - ${songTitle}`;
  else if (songTitle) fileName = songTitle;
  else if (audioFile.name) fileName = audioFile.name.replace(/\.[^/.]+$/, '');

  if (fileName.toLowerCase().endsWith('.lsk')) fileName = fileName.slice(0, -4);
...
export const getLineStartTime = (line: SyncLine): number => {
    ...
    const times = [...line.time, ...line.adlibs.flatMap(a => a.time)].filter((t): t is number => t !== null);
...
export const getAdlibStartTime = (adlib: import('../types').AdlibPart): number => {
    if (!adlib) return Infinity;
    const times = adlib.time.filter((t): t is number => t !== null);
    return times.length > 0 ? Math.min(...times) : Infinity;
};

export const getAdlibEndTime = (adlib: import('../types').AdlibPart): number => {
    if (!adlib) return Infinity;
    const times = adlib.time.filter((t): t is number => t !== null);
    return times.length > 0 ? Math.max(...times) : -Infinity;
};
...
export function exportToLrcFile(
  ...
  sortedSyncData.forEach(line => {
    ...
    for (let i = 0; i < line.chars.length; i++) {
        ...
        if (time !== null) lrcLine += `<${formatTimeForLrc(time)}>${char}`;
        else lrcLine += char;
    }
    lrcContent += lrcLine + '\n';
  });
